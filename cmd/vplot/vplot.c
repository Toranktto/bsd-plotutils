...
void getpict(void) {
  ...
  for (;;)
    ...
    case 'p':
      ...
      lastx = mapx(getw(stdin));
      lasty = mapy(getw(stdin));
      pl_point(lastx, lasty);
      pl_point(lastx + 1, lasty);
      pl_point(lastx, lasty + 1);
      pl_point(lastx + 1, lasty + 1);
      continue;
...
void plotch(register int c) {
  ...
  for (i = -16; i < 16; i += 2) {
    ...
    for (j = 7; j >= 0; --j)
      if ((c >> j) & 1) {
        pl_point(lastx + 6 - j * 2, lasty + i);
        pl_point(lastx + 7 - j * 2, lasty + i);
        pl_point(lastx + 6 - j * 2, lasty + i + 1);
        pl_point(lastx + 7 - j * 2, lasty + i + 1);
      }
  }
  lastx += 16;
}
