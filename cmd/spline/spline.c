...
static void getlim(struct proj *p);
static float rhs(int i);
static int spline(void);
...
rhs(int i)
{
	...
	return (
	    6 * ((y.val[i_ + 1] - y.val[i_]) / (x.val[i + 1] - x.val[i]) - zz));
}
...
spline(void)
{
	...
	for (i = n - !periodic; --i >= 0;) { /* back substitute */
		...
		hi1 = end ? x.val[1] - x.val[0] : x.val[i + 1] - x.val[i];
		D2yi1 = D2yi;
		if (i > 0) {
			hi = x.val[i] - x.val[i - 1];
			corr = end ? 2 * s + u : zero;
