...
struct proj {
	int		lbf, ubf, val_len;
	float		a, b, lb, ub, quant, mult, *val;
}		x, y;
float	       *diag, *r;
float		dx = 1.;
float		ni = 100.;
 ...
 * vector of differences hi=xi-xi-1 y"  be vector of 2nd derivs of approx
 * function If the points are numbered 0,1,2,...,n+1 then y" satisfies (R W
 * Hamming, Numerical Methods for Engineers and Scientists, 2nd Ed, p349ff)
 * hiy"i-1+2(hi+hi+1)y"i+hi+1y"i+1
 *
 * = 6[(yi+1-yi)/hi+1-(yi-yi-1)/hi]   i=1,2,...,n
 *
 * where y"0 = y"n+1 = 0 This is a symmetric tridiagonal system of the form
 *
 ...
 *
 * the back solution is y"n = rn/dn
 *
 ...
 *
 * unhappily it turns out that the recursion forward for d is quite strongly
 * geometrically convergent--and is wildly unstable going backward. There's
 * similar trouble with r, so the intermediate results must be kept.
 *
 ...
 * y0" = ky1", yn+1"   = kyn"
 *
 * for some constant k.  The above analysis was for k = 0; k = 1 fits
 * parabolas perfectly as well as stright lines; k = 1/2 has been recommended
 * as somehow pleasant.
 *
 ...
 * where h0=_ hn+1
 *
 * The same diagonalization procedure works, except for the effect of the 2
 * corner elements.  Let si be the part of the last element in the ith
 * "diagonalized" row that arises from the extra top corner element.
 *
 * s1 = h1
 *
 * si = -si-1hi/di-1	2<_i<_n+1
 *
 * After "diagonalizing", the lower corner element remains. Call ti the
 * bottom element that appears in the ith colomn as the bottom element to its
 * left is eliminated
 *
 * t1 = h1
 *
 * ti = -ti-1hi/di-1
 *
 * Evidently ti = si. Elimination along the bottom row introduces further
 * corrections to the bottom right element and to the last element of the
 * right hand side. Call these corrections u and v.
 *
 * u1 = v1 = 0
 *
 ...
 *
 * y = yix+ + yi+1x- -(h2i+1/6)[y"i(x+-x+3)+y"i+1(x--x-3)] where x+ = xi+1-x
 *
 * x- = x-xi
 */
