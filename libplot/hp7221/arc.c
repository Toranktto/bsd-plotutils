 ...
 * c^2 = a^2 + b^2 - 2*a*b*cos(angle) is used where "a" and "b" are the
 * radius of the circle and "c" is the distance between the beginning point
 * and the end point.
 *
...
pl_arc(int x, int y, int x0, int y0, int x1, int y1)
{
	...
	pl_move(x, y);
	/*
	 * Not quite implemented... printf("C(A%d c)[%d,%d]", angle, x0, y0);
	 */
}
